...
export async function getProcesses() {
  /** 
  try {
    // This gets the full object: { all:..., list: [...] }
    const processesObject = await si.processes();

    // Safely access the list, defaulting to an empty array if it doesn't exist
    const processList = processesObject?.list || [];

    // Map the process list to the exact format your frontend needs
    const cleanedProcessList = processList.map(p => ({
      pid: p.pid,
      name: p.name,
      // Map the library's 'pcpu' and 'pmem' to your frontend's 'cpu' and 'mem'
      cpu: Number(p.pcpu) || 0,
      mem: Number(p.pmem) || 0,
      // If the user is empty or null, default to 'SYSTEM'
      user: p.user || 'SYSTEM',
      started: p.started,
      command: p.command,
    }));
