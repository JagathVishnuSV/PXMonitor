...
async function updateProcessList() {
  try {
    ...
    // Optional: Log to the backend console to confirm it's working
    const activeProcess = cachedProcesses.find(p => p.cpu > 0);
    if (activeProcess) {
      console.log(`[Process Monitor] Update successful. Found active process: ${activeProcess.name} at ${activeProcess.cpu.toFixed(1)}% CPU.`);
    }
